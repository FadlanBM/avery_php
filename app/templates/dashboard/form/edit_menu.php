...
<body class="bg-background text-on-background min-h-screen" x-data="{ sidebarOpen: false }">
        ...
        <main class="flex-1 lg:ml-64 flex flex-col min-h-screen">
            ...
            <div class="p-4 lg:p-8 max-w-5xl mx-auto w-full">
                ...
                <div class="flex flex-col gap-8">
                    <!-- Intro Header -->
                    <div>
                        <h2 class="text-3xl font-bold font-headline text-orange-900 tracking-tight">Perbarui Hidangan</h2>
                        <p class="text-stone-500 mt-1">Ubah detail dan ketersediaan hidangan Anda.</p>
                    </div>
                    ...
                    <section class="bg-surface-container-lowest p-8 rounded-3xl shadow-[0_8px_24px_-4px_rgba(156,56,0,0.04)] border border-orange-50/50">
                        <form action="<?= BASE_URL ?>/dashboard/menu-management/edit" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="id" value="<?= $menu->id ?>" />
                            
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-bold text-orange-900 mb-2 font-headline ml-1">Nama Hidangan</label>
                                    <input name="name" value="<?= htmlspecialchars($menu->name) ?>" class="w-full bg-surface-container-high border-none rounded-xl px-4 py-3 text-on-surface placeholder:text-stone-400 shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" placeholder="Misal: Rendang Wagyu Saffron" type="text" required />
                                </div>
                                <div class="space-y-2">
                                    <label class="block text-sm font-bold text-orange-900 font-headline ml-1">Kategori</label>
                                    <select name="category" class="w-full bg-surface-container-high border-none rounded-xl px-4 py-3 text-on-surface shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" required>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input name="price" value="<?= (int)$menu->price ?>" class="w-full bg-surface-container-high border-none rounded-xl pl-12 pr-4 py-3 text-on-surface shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" placeholder="0" type="number" required />
                                    </div>
                                </div>
                                <div class="space-y-2">
                                    <label class="block text-sm font-bold text-orange-900 font-headline ml-1">Stok</label>
                                    <input name="stock" value="<?= (int)$menu->stock ?>" class="w-full bg-surface-container-high border-none rounded-xl px-4 py-3 text-on-surface shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" placeholder="0" type="number" min="0" required />
                                </div>
                                <div class="space-y-2 md:col-span-2">
                                    <label class="block text-sm font-bold text-orange-900 font-headline ml-1">Status Ketersediaan</label>
                                    <select name="status" class="w-full bg-surface-container-high border-none rounded-xl px-4 py-3 text-on-surface shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" required>
                                        <option value="available" <?= $menu->is_available ? 'selected' : '' ?>>Tersedia</option>
                                        <option value="unavailable" <?= !$menu->is_available ? 'selected' : '' ?>>Habis</option>
                                    </select>
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-bold text-orange-900 mb-2 font-headline ml-1">Deskripsi Menu</label>
                                    <textarea name="description" class="w-full bg-surface-container-high border-none rounded-xl px-4 py-3 text-on-surface placeholder:text-stone-400 resize-none shadow-sm focus:ring-1 focus:ring-primary focus:bg-white transition-all" placeholder="Ceritakan keistimewaan rasa, aroma, dan bahan baku hidangan ini..." rows="5"><?= htmlspecialchars($menu->description) ?></textarea>
                                </div>
                            </div>
                    </section>
                    ...
                    <section class="bg-surface-container-lowest p-8 rounded-3xl shadow-[0_8px_24px_-4px_rgba(156,56,0,0.04)] border border-orange-50/50">
                        ...
                        <!-- Existing Images -->
                        <?php 
                        $existingAssets = $menu->getAssets(); 
                        if (!empty($existingAssets)):
                        ?>
                            <div class="mb-8">
                                <h4 class="text-sm font-bold text-orange-900 mb-3 ml-1 font-headline">Gambar Saat Ini</h4>
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    <?php foreach ($existingAssets as $asset): ?>
                                        <div class="relative group aspect-square rounded-2xl overflow-hidden border border-orange-100 shadow-sm bg-stone-100">
                                            <img src="<?= BASE_URL . '/' . ltrim($asset['file_path'], '/') ?>" class="w-full h-full object-cover cursor-zoom-in" onclick="openPreview('<?= BASE_URL . '/' . ltrim($asset['file_path'], '/') ?>')">
                                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-3">
                                                <button type="button" onclick="openPreview('<?= BASE_URL . '/' . ltrim($asset['file_path'], '/') ?>')" class="w-10 h-10 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white backdrop-blur-sm transition-colors">
                                                    <span class="material-symbols-outlined text-xl">visibility</span>
                                                </button>
                                                <button type="button" onclick="confirmDeleteAsset(<?= $menu->id ?>, <?= $asset['id'] ?>)" class="w-10 h-10 bg-red-500/80 hover:bg-red-600 rounded-full flex items-center justify-center text-white backdrop-blur-sm transition-colors">
                        ...
                        <!-- Upload New Images -->
                        <div class="mb-4">
                            <h4 class="text-sm font-bold text-orange-900 mb-3 ml-1 font-headline">Tambah Gambar Baru</h4>
                        ...
                        <div id="previewModal" class="fixed inset-0 bg-black/90 z-[100] hidden items-center justify-center p-4" onclick="this.classList.add('hidden'); this.classList.remove('flex');">
                            ...
                            <button type="button" class="absolute top-6 right-6 text-white bg-white/20 p-2 rounded-full hover:bg-white/40 transition-colors">
                    ...
                    <div class="flex items-center justify-end gap-6 mt-4">
                        <button type="button" class="px-8 py-4 text-stone-500 font-bold hover:text-orange-900 transition-colors" onclick="window.location.href='<?= BASE_URL ?>/dashboard/menu-management'">
                            Batalkan
                        </button>
    ...
    <!-- Hidden Form for Asset Deletion -->
    <form id="delete-asset-form" action="<?= BASE_URL ?>/dashboard/menu-management/delete-asset" method="POST" class="hidden">
        <input type="hidden" name="menu_id" id="delete-asset-menu-id">
        <input type="hidden" name="asset_id" id="delete-asset-id">
    </form>
    ...
    <script>
                ...
                reader.onload = function(e) {
                    ...
                    div.innerHTML = `
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/60 to-transparent">
                            <span class="text-[10px] text-white font-bold px-2">Gambar Baru ${index + 1}</span>
                        </div>
                    `;
        ...
        function confirmDeleteAsset(menuId, assetId) {
            Swal.fire({
                title: 'Hapus Gambar?',
                ...
                if (result.isConfirmed) {
                    document.getElementById('delete-asset-menu-id').value = menuId;
                    document.getElementById('delete-asset-id').value = assetId;
